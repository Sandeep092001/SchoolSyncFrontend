...
const handleChange = (e: { target: { name: any; value: any; }; }) =>{
  ...
  const updateUserDetails = async(e: { preventDefault: () => void; }) =>{
    ...
    try {
        const response = await fetch(`http://localhost:8080/updateUserDetails/${localStorage.getItem('username')} `, {
            method: 'PUT',
            headers: {
  ...
  const getUserProfileDetails = async () => {
    try {
      const response = await fetch(`http://localhost:8080/getUserDetails/${localStorage.getItem('username')}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
